...
a,y = np.unique(boxdata[:,5],return_inverse=True)
print(a)
print(y)
inputy = np_utils.to_categorical(y, 2)
print(inputy)
...
#safety helmet classifier model
...
helmetclassifier = Model(inputs = inputim, outputs = dense1)


print(helmetclassifier.summary())

helmetclassifier.compile(optimizer = 'adam', loss = 'categorical_crossentropy', metrics = ['accuracy'])



#train
helmetclassifier.fit(X_train, y_train, batch_size = 40, epochs = 100, validation_split=0.3, verbose =1)



#save the model
helmetclassifier.save('helmetclassifier.h5')
